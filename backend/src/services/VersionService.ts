  ...
  async checkForConflict(
    ...
    const conflict = await client.entityConflict.create({
      data: {
        ...
        incomingData: (previousVersion?.data ??
          incomingData) as Prisma.InputJsonValue,
        workspaceId,
      },
    });

    // Emit conflict event for subscriptions
    const { publishConflictDetected } =
      await import('../resolvers/SubscriptionResolver');
    ...
    throw new ConflictError('Entity has been modified by another user', {
      ...
      expectedVersion,
      currentData,
      incomingData: (previousVersion?.data ?? incomingData) as Record<
        string,
        unknown
      >,
    });
