...
export async function handleGetTransactions(
  ...
  if (input.after) {
    try {
      const cursorData = JSON.parse(Buffer.from(input.after, 'base64').toString('utf-8')) as {offset: number; orderBy?: string};
      ...
      try {
        offset = Number.parseInt(Buffer.from(input.after, 'base64').toString('utf-8'), 10);
      } catch {
        offset = 0;
      }
    }
    ...
    try {
      const cursorData = JSON.parse(Buffer.from(input.before, 'base64').toString('utf-8')) as {offset: number; orderBy?: string};
      const beforeOffset = cursorData.offset ?? 0;
      offset = Math.max(0, beforeOffset - (input.last ?? 20));
    } catch {
      try {
        const beforeOffset = Number.parseInt(Buffer.from(input.before, 'base64').toString('utf-8'), 10);
        offset = Math.max(0, beforeOffset - (input.last ?? 20));
      } catch {
        offset = 0;
      }
  ...
  if (input.accountId) {
    const account = await accountRepository.findById(input.accountId, workspaceId, {id: true});
    if (!account) {
  ...
  // Try cache first
  const cached = await postgresCache.get<{items: Transaction[]; totalCount: number}>(cacheKey);
  if (cached) {
    const hasMore = offset + limit < cached.totalCount;
    const nextCursor = hasMore
      ? Buffer.from(JSON.stringify({offset: offset + limit, orderBy: input.orderBy ? `${input.orderBy.field}:${input.orderBy.direction}` : undefined}), 'utf-8').toString('base64')
      : null;
    return {
  ...
  void postgresCache.set(cacheKey, {items, totalCount}, CACHE_TTL_MS, cacheTags).catch(() => {
    // Ignore cache errors
  });

  const hasMore = offset + limit < totalCount;
  const nextCursor = hasMore
    ? Buffer.from(JSON.stringify({offset: offset + limit, orderBy: input.orderBy ? `${input.orderBy.field}:${input.orderBy.direction}` : undefined})).toString('base64')
    : null;
