      ...
      async () => {
        ...
        if (validatedInput.accountIds && validatedInput.accountIds.length > 0) {
          const accountPromises = validatedInput.accountIds.map(
            async (id: string) =>
              accountRepository.findById(id, workspaceId, { id: true })
          );
        ...
        if (
          ...
          validatedInput.categoryIds.length > 0
        ) {
          const categoryPromises = validatedInput.categoryIds.map(
            async (id: string) =>
              categoryRepository.findById(id, workspaceId, { id: true })
          );
          const categories = await Promise.all(categoryPromises);
          if (categories.some((c) => c === null)) {
            throw new ValidationError(
              'One or more categories not found or not accessible'
            );
          }
        }
        ...
        if (validatedInput.payeeIds && validatedInput.payeeIds.length > 0) {
          const payeePromises = validatedInput.payeeIds.map(
            async (id: string) =>
              payeeRepository.findById(id, workspaceId, { id: true })
          );
          const payees = await Promise.all(payeePromises);
          if (payees.some((p) => p === null)) {
            throw new ValidationError(
              'One or more payees not found or not accessible'
            );
          }
        }
        ...
        if (cachedAggregations) {
          ...
          const [totalCountResult, totalAmountResult, incomeExpenseTotals] =
            await Promise.all([
