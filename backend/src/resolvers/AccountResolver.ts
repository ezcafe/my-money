...
export class AccountResolver extends BaseResolver {
  ...
  async createAccount(
    _: unknown,
    {input}: {input: {name: string; initBalance?: number; accountType?: string; workspaceId?: string}},
    context: GraphQLContext,
  ): Promise<AccountWithNumericBalance> {
    // Get workspace ID from input or context (default to user's default workspace)
    const workspaceId = input.workspaceId ?? context.currentWorkspaceId ?? await getUserDefaultWorkspace(context.userId);
  ...
  async updateAccount(
    _: unknown,
    {id, input}: {id: string; input: {name?: string; initBalance?: number; accountType?: string; expectedVersion?: number}},
    context: GraphQLContext,
  ): Promise<AccountWithNumericBalance> {
    // Get workspace ID from context (default to user's default workspace)
    const workspaceId = context.currentWorkspaceId ?? await getUserDefaultWorkspace(context.userId);
    ...
    await UnitOfWork.create(context.prisma, async (uow) => {
      const txVersionService = getContainer().getVersionService(uow.getTransaction());
    ...
    const accountWithBalance = await accountRepository.findById(id, workspaceId);
   ...
   */
  async versions(parent: Account, _: unknown, context: GraphQLContext): Promise<unknown> {
    const versionService = getContainer().getVersionService(context.prisma);
   ...
   */
  async createdBy(parent: Account, _: unknown, context: GraphQLContext): Promise<unknown> {
   ...
   */
  async lastEditedBy(parent: Account, _: unknown, context: GraphQLContext): Promise<unknown> {
